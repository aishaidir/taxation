...
const { SERVER_ERROR, NETWORK_FAILURE, SIGN_UP_ERROR, TIMEOUT } = ERROR_MESSAGE;
export const completeUserSignUp = credentials => dispatch => {
      ...
      return timeout(10000, signUpCompletion(credentials))
        ...
        .catch(err => {
          ...
          if (err === TIMEOUT)
            return dispatch(
              dispatchAndRefresh(
                fetchCompleteSignUpError(TIMEOUT),
                fetchRefreshRequest()
              )
            );
          if (err.status === status.NOT_FOUND)
            return dispatch(
              dispatchAndRefresh(
                fetchCompleteSignUpError(SIGN_UP_ERROR.REQUEST_FAILED)
              )
            );
          if (err.status === status.NOT_ACCEPTABLE)
            return dispatch(
              dispatchAndRefresh(
                fetchCompleteSignUpError(SIGN_UP_ERROR.INCOMPLETE),
                fetchRefreshRequest()
              )
            );
          if (err.status === status.INTERNAL_SERVER_ERROR)
            return dispatch(
              dispatchAndRefresh(
                fetchCompleteSignUpError(SERVER_ERROR),
                fetchRefreshRequest()
              )
            );
          return dispatch(
            dispatchAndRefresh(
              fetchCompleteSignUpError(NETWORK_FAILURE),
              fetchRefreshRequest()
            )
          );
        });
